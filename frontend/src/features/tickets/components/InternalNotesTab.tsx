  ...
  return (
    <div className="space-y-4">
      <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs font-semibold flex items-center gap-2">
        ...
        <span>Internal notes are strictly private to support agents and business admins. Customers cannot see these messages.</span>
      </div>

      <div className="space-y-3">
        {notes.map((n) => (
          <div key={n.id} className="p-4 rounded-xl bg-amber-50/50 border border-amber-200 text-sm space-y-1 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        />
        <Button type="submit" variant="primary" size="md" className="bg-amber-600 hover:bg-amber-700">
